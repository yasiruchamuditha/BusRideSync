...
const LostItemCard = ({ item }) => {
  const handleContactAdmin = () => {
    ...
    const subject = `Inquiry about lost item: ${item.name}`;
    const body = `Hello Admin,\n\nI would like to inquire about the lost item with the following details:\n\nName: ${item.name}\nLost at: ${item.lostPlace}\nColor: ${item.color}\nSize: ${item.size}\nType: ${item.type}\nBus Number: ${item.busNumber}\nRoute: ${item.route}\n\nThank you.`;
    window.location.href = `mailto:${adminEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };

  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg m-4">
      {item.photos.length > 0 && <Image className="w-full" src={item.photos[0]} alt="Lost item" />}
      <div className="px-6 py-4">
        <div className="font-bold text-xl mb-2">{item.name}</div>
        <p className="text-gray-700 text-base">Lost at: {item.lostPlace}</p>
