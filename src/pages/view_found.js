...
export default function ViewFoundItems() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setFoundItems(foundItems.filter(item => item._id !== id)); // Update state after deletion
  ...
  return (
    <div className="min-h-screen bg-green-100 flex flex-col items-center py-8">
      <h2 className="text-3xl font-bold text-green-700 mb-6">View Found Items</h2>
        ...
        <table className="min-w-full bg-white border-collapse w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 border-b-2 border-gray-300">Found Place</th>
          ...
          <tbody>
            {foundItems.map((item) => (
              <tr key={item._id}>
                ...
                <td className="px-4 py-2 border-b border-gray-300">{item.foundPlace}</td>
